...
import com.asianpaints.facades.contactno.AsianpaintsChangeContactNofacades;
...
import com.asianpaints.storefront.validators.AsianpaintsContactValidator;
...
public class AccountPageController extends AbstractSearchPageController
{
	...
	private static final String REDIRECT_TO_UPDATE_CONTACTNO_PAGE = REDIRECT_PREFIX + "/my-account/update-contactNo";
	...
	private static final String UPDATE_CONTACTNO_CMS_PAGE = "update-contactNumber";
	...
	@Resource(name = "asianpaintsContactValidator")
	private AsianpaintsContactValidator asianpaintsContactValidator;

	@Resource(name = "asianpaintsChangeContactNofacades")
	private AsianpaintsChangeContactNofacades asianpaintsChangeContactNofacades;
	...
	@RequestMapping(value = "/update-contactNo", method = RequestMethod.GET)
	@RequireHardLogIn
	public String contactNo(final Model model) throws CMSItemNotFoundException
	{
		final CustomerData customerData = customerFacade.getCurrentCustomer();
		final AsianpaintsUpdateContactNoForm updateContactNoForm = new AsianpaintsUpdateContactNoForm();

		updateContactNoForm.setContactNo(customerData.getContactNo());

		model.addAttribute("asianpaintsUpdateContactNoForm", updateContactNoForm);

		storeCmsPageInModel(model, getContentPageForLabelOrId(UPDATE_CONTACTNO_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(UPDATE_CONTACTNO_CMS_PAGE));
		model.addAttribute("breadcrumbs", accountBreadcrumbBuilder.getBreadcrumbs("text.account.profile"));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		return getViewForPage(model);
	}

	@RequestMapping(value = "/update-contactNo", method = RequestMethod.POST)
	@RequireHardLogIn
	public String updatecontactNo(final AsianpaintsUpdateContactNoForm updateContactNoForm, final BindingResult bindingResult,
			final Model model, final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{

		asianpaintsContactValidator.validate(updateContactNoForm, bindingResult);
		String returnAction = REDIRECT_TO_UPDATE_CONTACTNO_PAGE;

		if (!bindingResult.hasErrors() && !updateContactNoForm.getContactNo().equals(updateContactNoForm.getChkcontactNo()))
		{
			bindingResult.rejectValue("chkcontactNo", "validation.checkContactNo.equals", new Object[] {},
					"validation.checkContactNo.equals");
		}

		if (bindingResult.hasErrors())
		{
			returnAction = setErrorMessagesAndCMSPage(model, UPDATE_CONTACTNO_CMS_PAGE);
		}
		else
		{
			try
			{
				asianpaintsChangeContactNofacades.changeContactNo(updateContactNoForm.getContactNo(),
						updateContactNoForm.getPassword());
				GlobalMessages.addFlashMessage(redirectAttributes, GlobalMessages.CONF_MESSAGES_HOLDER,
						"text.account.profile.confirmationUpdated", null);

				/*
				 * // Replace the spring security authentication with the new UID final String newUid =
				 * customerFacade.getCurrentCustomer().getUid().toLowerCase(); final Authentication oldAuthentication =
				 * SecurityContextHolder.getContext().getAuthentication(); final UsernamePasswordAuthenticationToken
				 * newAuthentication = new UsernamePasswordAuthenticationToken(newUid, null,
				 * oldAuthentication.getAuthorities()); newAuthentication.setDetails(oldAuthentication.getDetails());
				 * SecurityContextHolder.getContext().setAuthentication(newAuthentication);
				 */
			}
			/*
			 * catch (final DuplicateUidException e) { bindingResult.rejectValue("contactNo", "profile.contactNo.unique");
			 * returnAction = setErrorMessagesAndCMSPage(model, UPDATE_CONTACTNO_CMS_PAGE); }
			 */
			catch (final PasswordMismatchException passwordMismatchException)
			{
